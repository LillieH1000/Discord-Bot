...
class tweaksearch(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith("[[") & message.content.endswith("]]"):
            ...
            response = requests.get(f"https://api.canister.me/v1/community/packages/search?query={urllib.parse.quote(message_content)}&limit=1&responseFields=name,description,author,price,packageIcon,depiction,repository.name,repository.uri").json()
